...
namespace UserManagement_API.Services
{
    public class UserService : IUserService
    {
        ...
        // trong class UserService : IUserService
            ...
            UserWithInfoCreateDto dto, CancellationToken ct = default)
        {
            ...
            if (existing != null)
            {
                // 1a) ĐÃ xác thực -> báo controller trả 409
                if (existing.is_verified)
                    throw new InvalidOperationException("EMAIL_EXISTS");

                // 1b) CHƯA xác thực -> resend OTP (tôn trọng rate limit)
            ...
            <p>If you did not request this, please ignore.</p>";
                ...
                return (MapToReadDto(saved), true); // IsResend = true
            }

            // 2) Email CHƯA tồn tại -> tạo mới
            if (string.IsNullOrWhiteSpace(dto.Password))
                throw new InvalidOperationException("PASSWORD_REQUIRED"); // để controller trả 400
            ...
            var u = new User
            {
                ...
                PasswordHash = passwordHash,       // ✅ BẮT BUỘC
                is_verified = false,
                IsActive = false,
                CreatedAt = DateTime.UtcNow,

                // nếu DTO có các cờ role thì map luôn (tuỳ schema của bạn)
            ...
            var created = await _repo.CreateWithInfoAsync(u, ct);      // Nếu đã thêm CreateWithInfoAsync

            // Gửi email OTP sau khi Save thành công
